...
@pytest.mark.asyncio
async def test_human_funnel_never_leaks_protected_origin_to_declared_ai(tmp_path):
    ...
    async def origin_transport(request):
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://origin.test") as client:
            return await client.request(request.method, request.url.path, headers=request.headers,
    ...
    with sqlite3.connect(policy_db) as connection:
        ...
        connection.execute('INSERT INTO sites VALUES (?,?,?,?,NULL)', (
            'demo', 'gateway.test', 'http://origin.test',
            json.dumps({'strictness': 'balanced', 'routes': [
                {'pattern': '/protected', 'action': 'human'}
            ]})))

    settings = Settings(site_id='demo', dashboard_db=str(policy_db), origin_url='http://origin.test',
                        origin_secret=secret, dev_access_token='')
