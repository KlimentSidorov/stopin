...
def collect_signals(request, settings, sessions, evidence):
    ...
    session_id = verify_access_token(token, settings.token_secret, settings.site_id) if token else None
    ...
    return Signals(
        ...
        tripwires={"activated": bool(record and record.tripwire_hits)},
