...
def create_app(settings: Settings | None = None,
    ...
    @app.api_route("/challenge/tripwire", methods=["GET", "HEAD", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"])
    @app.api_route("/challenge/tripwire/{tripwire_id:path}", methods=["GET", "HEAD", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"])
    ...
    @app.api_route("/challenge/experiments", methods=["GET", "HEAD", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"])
    @app.api_route("/challenge/experiments/{resource_id:path}", methods=["GET", "HEAD", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"])
    def experiment_resource(request: Request, resource_id: str = ""):
        session_id = interaction_session(request)
        status, interaction = app.state.evidence_store.activate_experiment(session_id, resource_id, request.method)
        if interaction:
    ...
    def verify_challenge_endpoint(request: Request, body: ChallengeSubmission):
        ...
        if (not secrets.compare_digest(bound_session.encode(), body.session_id.encode())
                or not verify_challenge(body.challenge_id, bound_session, body.nonce, store=app.state.challenge_store)):
            ...
            record_event(request_id=str(uuid4()), site_id=settings.site_id, method="POST", path="/challenge/verify",
                         decision="BLOCK", reasons=["invalid_challenge"], session_id=bound_session, signals=interaction)
            return Response(status_code=403, headers=PRIVATE_HEADERS)
        ...
        interaction = app.state.evidence_store.lifecycle(bound_session, "session_issued" if decision is Decision.ALLOW else "verification_denied")
        if interaction:
            signals.behavior.update(interaction["behavior"])
        record_event(request_id=str(uuid4()), site_id=settings.site_id, method="POST", path="/challenge/verify",
                     decision=decision.value, reasons=reasons, session_id=bound_session, signals=signals.to_dict())
        ...
        response.delete_cookie("gateway_challenge_session", path="/", secure=settings.cookie_secure,
    ...
    async def gateway(request: Request):
        ...
        try:
            ...
            await run_in_threadpool(record_event, request_id=request_id, site_id=settings.site_id,
                                    method=request.method, path=request.url.path, decision="BLOCK",
                                    reasons=["dashboard_configuration_unavailable"])
        ...
        if decision is Decision.CHALLENGE:
            ...
            try:
                ...
                return Response(status_code=502, headers={**PRIVATE_HEADERS, "x-request-id": request_id})
